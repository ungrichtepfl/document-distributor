...
class DocumentDetailEntry(Frame):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.label_document_file_type.grid(row=0, column=0)
        self.entry_document_file_type = Entry(self, textvariable=self._document_file_types)
        self.entry_document_file_type.grid(row=0, column=1)
...
class FolderSelect(Frame):
    ...
    def __init__(self, parent=None, folder_description="", **kwwargs):
        ...
        self.button_find.grid(row=1, column=1)
...
def gui_root() -> tk.Tk:
    ...
    document_dir_select = FolderSelect(root, "Select Document Directory:")
    document_dir_select.grid(row=0)
    document_detail_entries = DocumentDetailEntry(root)
    document_detail_entries.grid(row=1)
    name_email_file_select = FileSelect(root,
                                        "Select name and email file:",
                                        filetypes=(*SUPPORTED_FILE_TYPES, ("All files", "*.*")))
    name_email_file_select.grid(row=2)
    name_email_details_entries = NameEmailDetailEntries(root)
    name_email_details_entries.grid(row=3)
