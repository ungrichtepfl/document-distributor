...
from document_distributor.document_distributor import map_document_to_names_and_emails
from document_distributor.document_distributor import map_name_and_email_to_document
...
def test_document_paths():
    document_file_paths = list_document_file_paths(
        "tests/test_data/test_documents", [".pdf"]
    )
...
@pytest.mark.skipif(
    os.environ.get("DDIST_SEND_EMAIL") is None, reason="Test sending real emails"
)
def test_send_email(msg, subject, attachments):
    send_email(
        sender_email=os.environ.get("DDIST_EMAIL_USERNAME"),
...
def test_email_starttsl_authenticated(
    smtpd, msg, sender_email, receiver_emails, subject, attachments
):
...
def test_name_to_documents_and_vice_versa():
    ...
    name_to_documents = map_name_and_email_to_document(
        document_file_paths, name_to_mail
    )
    pprint(name_to_documents)
    document_to_names = map_document_to_names_and_emails(
        document_file_paths, name_to_mail
    )
